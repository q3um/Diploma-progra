...
namespace ParserAndForms
{
    ...
    {
        public const string CompanyNamePattern = @"(((ОБЩЕСТВО С ОГРАНИЧЕННОЙ ОТВЕТСТВЕННОСТЬЮ)|(ООО Фирма)|(ТК)|(ПАО)|(ООО)|(ЗАО)|(АО)|(AO)|(ОАО)|(ТОО)|(ФГУП)|(ТД НЧ)|(ТД)|(ООО ПКФ)|(МУП)|(НПК)|(МП))\s((""|«|“)\d{0,}\D{2,}\d{0,}(""|»|”),{0,2}))|^(\D{0,33}(,))|((ИП)\s\D{3,}[.])|(^[А-я]{3,14}\s[А-я]{3,14}\s[А-я]{3,14}(,)?)|^([А-я]{3,14}\s[А-я]{3,14}\s[А-я]{3,14}\s[А-я]{3,14}(,)?)|^(\D{0,}(Ltd)|(LTD))";
